...
WarpSpec_RegBuffer_Kernel_21(data_type* d_A, data_type* d_B,  data_type* d_C, datasize_type Arows, datasize_type Bcols, datasize_type AcolsBrows, data_type alpha, data_type beta) {
 ...
 ,  &(reinterpret_cast<float4 *>(&d_A[gidx + (k + threadIDY2 + (trip_number * fake_blocksizey)) * Arows])[0]) , sizeof(float4)); 
    ...
    int regA_compute_offset = 0;
    int regB_compute_offset = 0;
              
            
   #pragma unroll
    for (int j = 0; j < TY; j++) {
       Reg_B[j] = AB_smem[COMPUTE_OFFSET + A_SMEM_SIZE + j * B_BATCHSIZE + 0 + ( threadIDY * B_BATCHSIZE * TY )]; 
    }
          
    #pragma unroll
    for (int i = 0; i < TX; i++) {
       Reg_A[i] = AB_smem[COMPUTE_OFFSET + threadIDX + 0 * 32 + (i * A_SMEM_BATCHSIZE)];  
    }
    
    
    int regA_copy_offset = TX;
    int regB_copy_offset = TY;

    #pragma unroll 
    for (int m = 1; m < B_BATCHSIZE; m++) {         
           
         #pragma unroll
         for (int j = 0; j < TY; j++) {
            Reg_B[regB_copy_offset + + j] = AB_smem[COMPUTE_OFFSET + A_SMEM_SIZE + j * B_BATCHSIZE + m + ( threadIDY * B_BATCHSIZE * TY )]; 
         }
          
         #pragma unroll
	for (int i = 0; i < TX; i++) {
 	    Reg_A [regA_copy_offset + + i] = AB_smem[COMPUTE_OFFSET + threadIDX + m * 32 + (i * A_SMEM_BATCHSIZE)];  
	}
	...
	for (int j = 0; j < TY; j++) {
            ...
            for (int i = 0; i < TX; i++) {                   
		int ind = i * TY + j;
                  sum[ind] = Reg_A[i + regA_compute_offset] * Reg_B[j + regB_compute_offset] + sum[ind];  

	    }

	}	
        
         regA_compute_offset  = ( regA_compute_offset != 0) ? 0 : TX;
         regB_compute_offset  = ( regB_compute_offset != 0) ? 0 : TY;
       
         regA_copy_offset  = ( regA_copy_offset != 0) ? 0 : TX;
         regB_copy_offset  = ( regB_copy_offset != 0) ? 0 : TY;	
    }  
