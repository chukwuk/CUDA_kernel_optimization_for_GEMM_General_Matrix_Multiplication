...
//  fprintf (stderr, "Amount of data transfered to the device is %lld GB\n", bytes4euc/1000000000);
   ...
   const int NUMBLOCKSX = (Arows + BLOCKSIZEX-1)/BLOCKSIZEX;
   const int NUMBLOCKSY =  (Bcols + BLOCKSIZEY-1)/BLOCKSIZEY;
  ...
  // call the kernel
  naive2D_GEMM_Kernel_2<data_type, datasize_type><<< grid, threads >>>( d_A, d_B, d_C, Arows, Bcols, AcolsBrows, alpha, beta);
